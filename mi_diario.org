...
* Temp
** Instalando debian 11 en Raspberry Pi 3b+
- Descargar [[https://raspi.debian.net/tested-images/][imagen]]
- Tostar imagen con RPI imager
- Buscar la ip de la raspberry con ‘sudo nmap -sn 192.168.1.0/24’ o 'sudo nast -m'
- Con el teclado y monitor conectado
  - Logearse como 'root' sin contraseña
  - Crear un nuevo usuario 'pi' con 'add user pi'
  - Buscar la ip local asignada con 'ip address'
- Conectarse desde una terminal remota con 'ssh pi@xxx.xxx.xxx.xxx'
- cambiar al usuario 'root' mediante 'su root'
- Poner el sistema en castellano '# export LANG=es_ES.UTF-8'
- Actualizar el sistema con 'apt update & apt upgrade -y'
- Instalar 'sudo' mediante 'apt sudo'
- Añadir al usuario 'pi' al grupo de sudoers 'sudo usermod -aG sudo pi'
- Instalar dependencias 'sudo apt-get install jq wget curl avahi-daemon udisks2 libglib2.0-bin network-manager dbus apparmor -y'
- Descargar el script de instalación de Docker 'curl -fsSL https://get.docker.com -o get-docker.sh'
- Convertir el archivo en ejecutable 'chmod +x get-docker.sh'
- Ejecutar el archivo './get-docker.sh'
- Añadir el usuario 'pi' al grupo de Docker 'sudo usermod -aG docker pi'
...
Links
https://www.bollyinside.com/articles/how-to-configure-and-turn-on-ssh-on-debian-11-bullseye-linux
https://vitux.com/shutdown-reboot-debian-11/
